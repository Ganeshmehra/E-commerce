...
const Sign_Up = () => {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("Data1")) || [];
    setArr(storedData);
  }, []);
  ...
  const handleClick = () => {
    if (validateData()) {
      ...
      if (FindData) {
        ...
        let get = JSON.parse(localStorage.getItem("Data1")) || []
        const copy = get.concat(data1)
        localStorage.setItem("Data1", JSON.stringify(copy));
  ...
  return (
    <>
      <div className='container w-45 rounded mx-auto text-center my-4 loginpage w-75 p-5'>
        <div className='my-3'>
        ...
        <div className='my-3'>
          <label htmlFor="password" className='d-flex justify-content-start text-info'>Address</label>
         <textarea className='p-3' name="Address" id="" onChange={handleChange} value={data1.Address} placeholder='Enter Address'>Address</textarea>
        </div>
